                        ...
                        <div className="flex flex-col items-center">
                            <div
                                className={`flex h-9 w-9 items-center justify-center rounded-full border transition-all sm:h-10 sm:w-10 ${
                                    isActive
                                        ? 'border-accent bg-accent-wash shadow-raised'
                                        : isCompleted
                                            ? 'border-covered bg-covered-wash'
                                            : 'border-rule bg-surface'
                                }`}
                            >
                                <Icon className={`h-4 w-4 sm:h-5 sm:w-5 ${
                                    isActive ? 'text-accent' : isCompleted ? 'text-covered' : 'text-neutral'
                                }`} />
                            </div>
                            {index < statusSteps.length - 1 && (
                                <div className={`h-10 w-0.5 sm:h-12 ${isCompleted ? 'bg-covered' : isActive ? 'bg-accent' : 'bg-surface-raised'}`} />
                            )}
                        </div>
                        <div className="flex-1 pb-6 sm:pb-8">
                            <h4 className={`text-sm font-bold sm:text-base ${isActive ? 'text-ink' : isCompleted ? 'text-covered' : 'text-neutral'}`}>
                                {step.label}
                            </h4>
                            {isActive && (
