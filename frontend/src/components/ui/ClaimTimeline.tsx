                        ...
                        <div className="flex flex-col items-center">
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all ${isActive
                                    ? 'bg-blue-50 border-blue-500 shadow-[0_4px_14px_0_rgba(59,130,246,0.39)]'
                                    : isCompleted
                                        ? 'bg-emerald-50 border-emerald-500'
                                        : 'bg-slate-100 border-slate-200'
                                    }`}
                            >
                                <Icon className={`w-5 h-5 ${isActive ? 'text-blue-600' : isCompleted ? 'text-emerald-500' : 'text-slate-400'}`} />
                            </div>
                            {index < statusSteps.length - 1 && (
                                <div className={`w-0.5 h-12 ${isCompleted ? 'bg-emerald-500/50' : 'bg-slate-200'}`} />
                            )}
                        </div>
                        <div className="flex-1 pb-8">
                            <h4 className={`font-bold ${isActive ? 'text-slate-900' : isCompleted ? 'text-emerald-700' : 'text-slate-500'}`}>
                                {step.label}
                            </h4>
                            {isActive && (
